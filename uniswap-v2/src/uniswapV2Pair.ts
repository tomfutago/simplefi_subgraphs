...
import { Burn, Mint, Sync, Transfer } from "../generated/templates/UniswapV2Pair/UniswapV2Pair";
import {
  ADDRESS_ZERO,
  getOrCreateAccount,
  investInMarket,
...
function createOrUpdatePositionOnMint(
  ...
  log.info("XXXXX createOrUpdatePositionOnMint at TX {}, source={}", [
  ...
  log.info("XXXX calculate token balances. pair.totalsupply={}", [pair.totalSupply.toString()]);
  ...
  log.info("XXXX token balances calculated", []);
  ...
  log.info("XXXX exit createOrUpdatePositionOnMint", []);
}
...
function checkIncompleteBurnFromLastTransaction(event: ethereum.Event, pair: PairEntity): void {
  log.info("XXXX checkIncompleteBurnFromLastTransaction at TX {}, source={}", [
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString(),
    event.address.toHexString(),
  ]);
  ...
  log.info("XXXX checkIncompleteBurnFromLastTransaction done!!", []);
}

export function handleTransfer(event: Transfer): void {
  log.info("XXXX handleTransfer at TX {}, source={}", [
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString(),
    event.address.toHexString(),
  ]);

  log.info("XXXX from={}; to={}, value={}", [
...
export function handleMint(event: Mint): void {
  log.info("XXXX handleMint at TX {}, source={}", [
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString(),
    event.address.toHexString(),
  ]);

  let pair = PairEntity.load(event.address.toHexString()) as PairEntity;
  log.info("XXXX loaded pair {}", [pair.id]);
  log.info("XXXX totalSupply={}", [pair.totalSupply.toString()]);
...
export function handleBurn(event: Burn): void {
  log.info("XXXX handleBurn at TX {}, source={}", [
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString(),
    event.address.toHexString(),
  ]);
...
export function handleSync(event: Sync): void {
  log.info("XXXX handleSync at TX {}, source={}", [
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString(),
    event.address.toHexString(),
  ]);
